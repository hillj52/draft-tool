...
async function createMissingRBs(db: PrismaClient, valueMap: { [playerHash: string]: number }) {
  const dalvinCook = await db.player.findFirst({ where: { name: 'Dalvin Cook' }});
  if (!dalvinCook) {
  ...
  const zeke = await db.player.findFirst({ where: { name: 'Ezekiel Elliott' }});
  if (!zeke) {
    await db.player.create({ data: {
  ...
  const leo = await db.player.findFirst({ where: { name: 'Leonard Fournette' }});
  if (!leo) {
    await db.player.create({ data: {
